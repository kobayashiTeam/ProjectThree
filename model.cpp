...
// 追加：マテリアルの複製処理
// 「このModelインスタンス専用のマテリアルが欲しい」ときに1回だけ呼ぶ。
// 各パーツのpMaterialを、Clone()で作った複製に差し替える。
void Model::CloneMaterialsForInstance(ID3D11Device* pDevice)
{
    for (auto& part : m_Parts)
    {
        ...
        if (clonedMat)
        {
            ...
            m_ownedClonedMaterials.push_back(clonedMat); // 解放責任をこのModelが持つ
        }
    }
}

// 追加：metallic/roughnessをこのModelの全パーツに適用する
// 呼び出し前にCloneMaterialsForInstance()していないと、他のModelとも共有している
// 元のマテリアルを直接書き換えてしまうので注意（他の個体も一緒に変わってしまう）
void Model::SetMetallicRoughness(float metallic, float roughness)
{
    for (auto& part : m_Parts)
    {
        // DeferredCBMaterialでない場合（LitMaterial等）は何もしない
