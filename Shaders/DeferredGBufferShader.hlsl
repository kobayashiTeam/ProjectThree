...
struct PS_OUTPUT
{
    float4 Color : SV_Target0; // RT0: アルベド(rgb) + metallic(a)
    float4 Normal : SV_Target1; // RT1: 法線(rgb) + roughness(a)
...
PS_OUTPUT PS(PS_INPUT input)
{
    ...
    // RT0 にマテリアル本来の色を書き込みalphaに metallic を格納する
    // (旧: output.Color = objectColor; のalpha値=テクスチャ由来のalphaは今回捨てる)
    ...
    // RT1 に法線を書き込みalphaに roughness を格納する
    // (旧: 1.0f固定で「将来のマテリアルID用」としていた枠を実際に使う)
