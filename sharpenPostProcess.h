...
class SharpenPostProcess : public PostProcess {
public:
    struct PerEffectCB {
        float intensity; // シャープの強さ (0.0 = 通常, 1.0 = 最大限に強調)
...
public:
    SharpenPostProcess() { m_cbData.intensity = 1.0f; }
    ~SharpenPostProcess() override { if (m_pConstantBuffer) m_pConstantBuffer->Release(); }
