...
// スロット2用の定数バッファを生成する
bool UnLitMaterial::CreateMaterialBuffer(ID3D11Device* pDevice)
{
...
// 描画時に呼ばれるバインド処理
void UnLitMaterial::Bind(ID3D11DeviceContext* pContext)
{
    if (!pContext) return;
    ...
    // 2. 自分専用（UnLitMaterial）のデータを定数バッファに書き込んで、スロット2にセット
